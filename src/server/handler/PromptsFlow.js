...
class ExplanationStep extends StepWithVote {
  ...
  nextStep() {
    this.handler.nextStep({ prompt: this.prompt, shots: this.shots });
  }
...
class InputStep extends Step {
  ...
  constructor(handler, room, { prompt, shots }) {
    super(room);
    this.handler = handler;
    this.prompt = prompt;
    this.shots = shots;
  ...
  nextStep() {
    this.handler.nextStep({ shots: this.shots, prompt: this.prompt, results: this.results });
...
class ResultStep extends Step {
  constructor(handler, room, { prompt, shots, results }) {
    super(room);
